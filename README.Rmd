...
[![CRAN downloads](https://cranlogs.r-pkg.org/badges/hexsession)](https://cran.r-project.org/package=hexsession)
[![CRAN downloads total](https://cranlogs.r-pkg.org/badges/grand-total/hexsession)](https://cran.r-project.org/package=hexsession)
...
The `make_tile()` function generates a self-contained HTML file and writes it to a `temp_hexsession/` subfolder inside `tempdir()` by default. The function returns the path to the HTML file and prints it as a message, so you always know where to find it. To write the output to your project directory instead, pass `output_dir = getwd()` (or any other path).
...
Set `highlight_mode` to `TRUE` if you want a tile in which all images are dimmed except the one being hovered over, to emphasize individual packages interactively: 
...
When making tiles for a vector of package names, use `focus` to specify packages one or more packages that will be highlighted by dimming all the other images in the tile.
...
This package depends on working installations of magick and chromote, and thus requires a Chromium-based web browser (e.g., Chrome, Chromium, Opera, or Vivaldi) for `snap_tile()`.
